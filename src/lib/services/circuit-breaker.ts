...
export class CircuitBreaker {
		...
		switch (this._state) {
			...
			case CircuitBreakerStates.OPEN:
				...
				if (this.lastFailureTime && Date.now() - this.lastFailureTime >= this.resetTimeout) {
					...
					console.log(`[CircuitBreaker] ${this.serviceId}: ABERTO -> MEIO_ABERTO (testando recuperação)`);
					return true;
				}
				return false;
	...
	recordSuccess(): void {
		...
		if (this._state === CircuitBreakerStates.HALF_OPEN) {
			...
			console.log(`[CircuitBreaker] ${this.serviceId}: MEIO_ABERTO -> FECHADO (recuperado)`);
		}
	...
	recordFailure(): void {
		...
		if (this._state === CircuitBreakerStates.HALF_OPEN) {
			...
			console.warn(`[CircuitBreaker] ${this.serviceId}: MEIO_ABERTO -> ABERTO (recuperação falhou)`);
		} else if (this.failures >= this.failureThreshold) {
			// Threshold reached - open the circuit
			this._state = CircuitBreakerStates.OPEN;
			console.warn(
				`[CircuitBreaker] ${this.serviceId}: FECHADO -> ABERTO (${this.failures} falhas)`
			);
		}
	}
	...
	reset(): void {
		...
		this.halfOpenRequests = 0;
		console.log(`[CircuitBreaker] ${this.serviceId}: Resetado para FECHADO`);
	}
}
