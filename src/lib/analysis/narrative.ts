/**
 * Rastreador de narrativas - analisa propagação de narrativas de fontes alternativas para mainstream
 */
...
export function getNarrativeSummary(results: NarrativeResults | null): {
	...
	if (!results) {
		return { total: 0, status: 'SEM DADOS' };
	}
	...
	return {
		total,
		status: total > 0 ? `${total} ATIVO` : 'MONITORANDO'
	};
}
