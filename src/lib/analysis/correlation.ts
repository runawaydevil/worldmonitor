/**
 * Motor de correlação - analisa padrões entre itens de notícias
 */
...
function getPrediction(topic: CorrelationTopic, count: number): string {
	if (topic.id === 'tariffs' && count >= 4) {
		return 'Volatilidade de mercado provável nas próximas 24-48h';
	}
	if (topic.id === 'fed-rates') {
		return 'Espere maior cobertura do setor financeiro';
	}
	if (topic.id.includes('china') || topic.id.includes('russia')) {
		return 'Narrativa de escalada geopolítica se formando';
	}
	if (topic.id === 'layoffs') {
		return 'Preocupações com emprego podem dominar o ciclo de notícias';
	}
	if (topic.category === 'Conflict') {
		return 'Desenvolvimentos importantes prováveis em horas';
	}
	return 'Tópico ganhando tração no mainstream';
}
...
export function getCorrelationSummary(results: CorrelationResults | null): {
	...
	if (!results) {
		return { totalSignals: 0, status: 'SEM DADOS' };
	}
	...
	return {
		totalSignals,
		status: totalSignals > 0 ? `${totalSignals} SINAIS` : 'MONITORANDO'
	};
}
