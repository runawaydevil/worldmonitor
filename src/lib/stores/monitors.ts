...
function loadMonitors(): CustomMonitor[] {
	...
	try {
		...
		console.warn('Falha ao carregar monitores do localStorage:', e);
		return [];
	}
}
...
function saveMonitors(monitors: CustomMonitor[]): void {
	...
	try {
		...
		console.warn('Falha ao salvar monitores no localStorage:', e);
	}
}
	...
	return {
		...
		addMonitor(
			...
			if (state.monitors.length >= MAX_MONITORS) {
				console.warn(`Número máximo de monitores (${MAX_MONITORS}) atingido`);
				return null;
			}
