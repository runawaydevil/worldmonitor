	...
	<div class="modal-overlay">
		<div class="modal onboarding-modal">
			<div class="modal-header">
				<button class="close-btn" onclick={handleClose} aria-label="Pular onboarding">
					&times;
				</button>
				<h2>Bem-vindo ao Watchman</h2>
				<p class="subtitle">Escolha uma configuração de dashboard para começar</p>
			</div>
			...
			<div class="preset-grid">
				{#each PRESET_ORDER as presetId}
					...
					<button class="preset-card" onclick={() => handleSelectPreset(presetId)}>
						...
						<div class="preset-panel-count">{preset.panels.length} painéis</div>
					</button>
				{/each}
			</div>

			<div class="modal-footer">
				<p class="hint">Você pode alterar isso depois nas Configurações</p>
			</div>
		</div>
	</div>
{/if}
