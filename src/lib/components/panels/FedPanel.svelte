...
<Panel id="fed" title="Federal Reserve" count={newsState.items.length} {loading} {error}>
	...
	{:else if !hasApiKey && !loading}
		<div class="no-api-key">
			<span class="no-api-key-text">Adicione VITE_FRED_API_KEY para indicadores econômicos</span>
		</div>
	{/if}
	...
	{#if videoItems.length > 0}
		<div class="video-section">
			<div class="section-header">
				<span class="section-title">Discursos e Vídeo</span>
				<a
	...
	<div class="news-section">
		{#if newsState.items.length === 0 && !loading && !error}
			<div class="empty-state">Nenhuma notícia do Fed disponível</div>
		{:else}
