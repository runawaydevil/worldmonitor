...
<Panel id="leaders" title={PANELS.leaders.name} {count} {loading} {error}>
	{#if leaders.length === 0 && !loading && !error}
		<div class="empty-state">Nenhum dado de líderes disponível</div>
	{:else}
			...
			{#each leaders as leader (leader.id)}
				...
				<div class="leader-card {activityClass}">
					...
					<div class="leader-meta">
						<span class="leader-since">Desde {leader.since}</span>
