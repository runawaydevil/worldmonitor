...
<Panel id="monitors" title={PANELS.monitors.name} {count} {loading} {error}>
	<div class="monitors-content">
		{#if monitors.length === 0 && !loading && !error}
			<div class="empty-state">
				<p>Nenhum monitor configurado</p>
				{#if onCreateMonitor}
					<button class="create-btn" onclick={onCreateMonitor}> + Criar Monitor </button>
				{/if}
			</div>
		{:else}
			<div class="monitors-header">
				<span class="active-count">{activeMonitors.length} ativo</span>
				{#if onCreateMonitor}
							...
							<div class="monitor-actions">
								{#if onToggleMonitor}
									<button
										...
										title={monitor.enabled ? 'Desabilitar' : 'Habilitar'}
									>
										{monitor.enabled ? '●' : '○'}
									</button>
								{/if}
								{#if onEditMonitor}
									<button class="action-btn" onclick={() => onEditMonitor?.(monitor)} title="Editar">
										✎
									</button>
								{/if}
								{#if onDeleteMonitor}
