...
<Panel id="narrative" title={PANELS.narrative.name} {loading} {error}>
	{#if news.length === 0 && !loading && !error}
		<div class="empty-state">Dados insuficientes para análise de narrativas</div>
	{:else if analysis}
		<div class="narrative-content">
			{#if analysis.emergingFringe.length > 0}
				<div class="section">
					<div class="section-title">Marginais Emergentes</div>
					{#each analysis.emergingFringe.slice(0, 4) as narrative}
						<div class="narrative-item">
							...
							<div class="narrative-meta">
								<span class="mention-count">{narrative.count} menções</span>
							</div>
			...
			{#if analysis.fringeToMainstream.length > 0}
				<div class="section">
					<div class="section-title">Cruzamentos Marginais → Mainstream</div>
					{#each analysis.fringeToMainstream.slice(0, 3) as crossover}
						<div class="crossover-item">
							...
							<div class="crossover-path">
								<span class="from">Marginal ({crossover.fringeCount})</span>
								<span class="arrow">→</span>
								<span class="to">Mainstream ({crossover.mainstreamCount})</span>
							</div>
							<div class="crossover-level">
								Nível de cruzamento: {Math.round(crossover.crossoverLevel * 100)}%
							</div>
						</div>
					{/each}
				</div>
			...
			{#if analysis.narrativeWatch.length > 0}
				<div class="section">
					<div class="section-title">Monitoramento de Narrativas</div>
			...
			{#if analysis.disinfoSignals.length > 0}
				<div class="section">
					<div class="section-title">Sinais de Desinformação</div>
					{#each analysis.disinfoSignals.slice(0, 3) as signal}
						<div class="disinfo-item">
							...
							<div class="disinfo-meta">{signal.count} menções</div>
						</div>
					{/each}
				</div>
			{/if}
			...
			{#if analysis.emergingFringe.length === 0 && analysis.fringeToMainstream.length === 0}
				<div class="empty-state">Nenhuma narrativa significativa detectada</div>
			{/if}
		</div>
	{:else}
		<div class="empty-state">Nenhuma narrativa significativa detectada</div>
