...
<Panel id="correlation" title={PANELS.correlation.name} {loading} {error}>
	{#if news.length === 0 && !loading && !error}
		<div class="empty-state">Dados insuficientes para análise</div>
	{:else if analysis}
		<div class="correlation-content">
			{#if analysis.emergingPatterns.length > 0}
				<div class="section">
					<div class="section-title">Padrões Emergentes</div>
					{#each analysis.emergingPatterns.slice(0, 3) as pattern}
						<div class="pattern-item">
							...
							<div class="pattern-sources">
								{pattern.sources.slice(0, 3).join(' · ')} ({pattern.count} itens)
							</div>
						</div>
					{/each}
				</div>
			...
			{#if analysis.momentumSignals.length > 0}
				<div class="section">
					<div class="section-title">Sinais de Momentum</div>
			...
			{#if analysis.predictiveSignals.length > 0}
				<div class="section">
					<div class="section-title">Sinais Preditivos</div>
			...
			{#if analysis.emergingPatterns.length === 0 && analysis.momentumSignals.length === 0}
				<div class="empty-state">Nenhum padrão significativo detectado</div>
			{/if}
		</div>
	{:else}
		<div class="empty-state">Nenhum padrão significativo detectado</div>
