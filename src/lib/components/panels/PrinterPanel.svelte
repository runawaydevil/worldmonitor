<script lang="ts">
	...
	const status = $derived(isExpanding ? 'IMPRESSORA LIGADA' : 'IMPRESSORA DESLIGADA');
	const statusClass = $derived(isExpanding ? 'critical' : 'monitoring');
</script>

<Panel id="printer" title={PANELS.printer.name} {status} {statusClass} {loading} {error}>
	{#if !data && !loading && !error}
		<div class="empty-state">Nenhum dado do Fed disponível</div>
	{:else if data}
		<div class="printer-gauge">
			<div class="printer-label">Balanço do Federal Reserve</div>
